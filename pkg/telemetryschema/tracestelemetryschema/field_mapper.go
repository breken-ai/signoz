...
func (m *storage) Read(ctx context.Context, q qbtypes.QueryInfo, key *telemetrytypes.TelemetryFieldKey) (qbtypes.Read, error) {
	...
	presence, ok := scopeJSONExistsExpression(key, sql, true)
	absence, _ := scopeJSONExistsExpression(key, sql, false)
	if !ok {
		if presence, err = querybuilder.ExistsExpression(columns, key, q.StartNs, q.EndNs, sql, true); err != nil {
			return qbtypes.Read{}, err
		}
		if absence, err = querybuilder.ExistsExpression(columns, key, q.StartNs, q.EndNs, sql, false); err != nil {
			return qbtypes.Read{}, err
		}
	}
	return qbtypes.Read{SQL: sql, Presence: presence, Absence: absence, WhenAbsent: whenAbsent, KeepType: temporal}, nil
}
