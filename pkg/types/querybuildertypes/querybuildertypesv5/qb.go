...
// QueryInfo is the query's context as one value: the time range every read
// needs, the signal and queried metric that family admission
...
type Read struct {
	...
	WhenAbsent Absent
	// KeepType: the stage cast must leave the native type alone (a time
	// column would collapse to seconds; a metrics label already reads as
	// String).
	KeepType bool
	// FilterOnly: the key filters but cannot be selected or grouped (the
	// legacy string body). The column expression builder drops such a
	// candidate, and errors only when none remains.
	FilterOnly bool
}
...
// Storage is what the query compiler knows about one signal's tables: the
// read of one key it cannot spell itself, one fallback it cannot guess, the
// traits it branches on, and the one compile step a storage with its own
// condition language overrides. Resolution, the condition builder, and the
// column expression builder live in querybuilder and are written once.
type Storage interface {
	// Read answers for one key from metadata or from Fallback: the bare
	// value read, the membership test, and what an absent row reads.
	// Virtual keys (a span scope) answer AlwaysPresent with the predicate
	// true.
	Read(ctx context.Context, q QueryInfo, key *telemetrytypes.TelemetryFieldKey) (Read, error)

	// Fallback returns the field keys that could hold a key metadata does not
	...
	// querybuilder.LogicalRead, applies the member's ValueMap itself: on the
	// read, or through StoredValues on the operand. Pattern operands (LIKE,
	// REGEXP, CONTAINS) have no stored translation: transform the read, or
	// drop that member's operand-side use (an index hint), never emit an
	// untranslated one.
